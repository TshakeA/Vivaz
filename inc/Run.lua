...
return sendAudioo(Chat_id,0,download_yt,(Info.title or '@Y_Q_8'),'@Y_Q_8','@Y_Q_8',function(a,d) if d.message_ == "File must be non-empty" then return sendMsg(Chat_id,0,' ✿ | لا يمكن تنزيل الفيديو للاسف  ') ,os.execute('rm -rf '..download_yt) else os.execute('rm -rf '..download_yt) end;end),os.execute('rm -rf '..download_yt)
end
end
